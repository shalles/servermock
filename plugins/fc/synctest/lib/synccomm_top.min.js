...
return false;}
var eventData={},eventID=0,eventDomID='synccommand_'+new Date().getTime(),notSendList=[{ele:['window','document'],type:['DOMContentLoaded','load']}];function buildCurrentEventObject(evt,tgt){evt.target=tgt;return evt;}
function parseCommand(command){command=JSON.parse(command);var ele,listeners,evt={},selector=command.selector;if(inList({ele:selector,type:command.type},notSendList))return{};ele=selector==='window'?window:document.querySelector(selector);listeners=eventData[ele[eventDomID]][command.type];buildCurrentEventObject(evt,ele); switch(command.type){case'input':ele.value=command.value;break;case'focus':ele.focus();break;case'scroll':ele.scrollTop?(ele.scrollTop=command.scrollTop,ele.scrollLeft=command.scrollLeft):window.scrollTo(command.scrollLeft,command.scrollTop);break;case'click':if(ele.tagName&&ele.tagName.toLowerCase()==="a"){ele.href.indexOf("javascript")===-1&&(location.href=ele.href);}
break;default:break;}
switch(ele.tagName&&ele.tagName.toLowerCase()){case'input':ele.value=command.value;break;default:break;}
return{event:evt,type:command.type,ele:ele,listeners:listeners}}
...
switch(ele.tagName&&ele.tagName.toLowerCase()){case'input':command.value=ele.value;break;default:break;}
...
function excuteCommand(command){var ele,type,event,listener;if(!(ele=command.ele))return;type="__synctest_event",event=new Event(type);listener=function(e){e.preventDefault();command.listeners&&command.listeners.fire(e);}
originAddEventListener.call(ele,type,listener);ele.dispatchEvent(event);ele.removeEventListener(type,listener);}
var originAddEventListener=EventTarget.prototype.addEventListener;window.addEventListener=EventTarget.prototype.addEventListener=function(type,listener,useCapture){var self=this;self[eventDomID]||(self[eventDomID]=++eventID);((eventData[eventID]||(eventData[eventID]={}))[type]||(eventData[eventID][type]=new Callbacks())).add(function(e){e.preventDefault();listener.call(self,e);});var callback=function(e){sendCommand(buildCommand(this,e));listener.call(this,e);}
